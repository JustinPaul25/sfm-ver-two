...
class SamplingController extends Controller
{
    ...
    public function report(Request $request)
    {
        ...
        if ($samplingId) {
            ...
            if ($sampling) {
                ...
                $historicalData = $historicalSamplings->map(function ($s, $index) use ($numberOfFish, &$previousBiomass, &$previousDate) {
                    $samples = $s->samples;
                    $totalWeight = $samples->sum('weight');
                    $totalSamples = $samples->count();
                    $avgWeight = $totalSamples > 0 ? round($totalWeight / $totalSamples, 2) : 0;
                    ...
                    // Calculate total weight gained (biomass difference from previous sampling) for FCR
                    $totalGained = 0;
    ...
    private function exportMockReport()
    {
        ...
        $historyHeaders = [
            ...
            'Present Stocks (pcs)', 'ABW (grams)', 'Biomass (kgs)', 'Feeding Rate',
            'Daily Feed Ration (kgs)', 'Feed Consumed (kgs)', 'FCR',
        ];
        ...
        $historyData = [
            ['05-Sep-24', 1, 5000, 0, 5000, 8, 40, '8%', 40, 17, 0],
            ['Oct 25, 2024', 52, 5000, 12, 4988, 48, 239, '5%', 12, 330, 1.7],
            ['Nov 29, 2024', 112, 5000, 30, 5000, 161, 803, '4%', 32, 375, 2.0],
            ['Jan 22, 2025', 176, 5000, 30, 5000, 225, 1127, '3%', 32, 1080, 2.0],
        ];
